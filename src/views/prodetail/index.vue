...
export default {
  ...
  methods: {
    ...
    async getComments () {
      const { data: { list, total } } = await getProComments(this.$route.params.id, 3)
      this.list = list
      this.total = total
    },
    addFn () {
    ...
    async addCart () {
      ...
      const { data: { cartTotal } } = await addCart(this.$route.params.id, +this.addCount, this.detail.skuList[0].goods_sku_id)
